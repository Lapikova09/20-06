...
public:
	...
	void hpChange(int fhp);

	void pointChange(int fpoint);
	...
	int getCollidedFish() { return collidedFish; }
...
void  Player::hpChange(int fhp) {
	hp = hp - fhp;
}


void  Player::pointChange(int fpoint) {
	point = point + fpoint;
}
...
void::Player::collidedFishChange() {
	collidedFish = collidedFish + 1;
}
